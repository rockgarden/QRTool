...
/*
 CIImage 带便图像的对象
 CIFilter 表示滤镜，使用key-value coding设置输入值，滤镜强度和输入的CIImage 包含了一个对输入图像的引用以及需要应用于数据的滤镜
 CIContext 用于渲染CIImage，当一个图片被渲染，作用于图片的滤镜链会被应用到原始的图像数据上。 CIContext可以是基于CPU的，输出为CGImageRef，也可以是基与GPU的，开发者可通过Open ES 2.0 画出来
 CIDetector 用于分析CIImage，以得到CIFeature，每个CIDetector都要用一个探测器类型（NSString）来初始化。这个类型用于告诉探测器要找什么特征
 CIFeatureCIFaceFeature 当一个CIDetector 分析一个图像，返回值是一个根据探测器类型探测到的CIFeature数组。每一个CIFaceFeature都包含了一个面部的CGRect引用，以及检测到的面孔的左眼、右眼、嘴部对应的CGpoint
 */
